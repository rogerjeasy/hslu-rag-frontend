...
import { DetailLevel } from '@/types/study-guide';
import { DifficultyLevel } from '@/types/practice-questions-old';
...
interface ConversationState {
  ...
  startPracticeQuestionsConversation: (
    courseId: string,
    topic: string,
...
export const useConversationStore = create<ConversationState>((set, get) => ({
  ...
  startStudyGuideConversation: async (
    courseId, 
    topic, 
    ...
    try {
      const conversation = await conversationService.startStudyGuideConversation(
        courseId, topic, detailLevel as 'basic' | 'medium' | 'comprehensive', 
        format, moduleId, topicId
      );
      
      // Update the conversation lists
      set(state => {
        // Create a summary for the list
        const conversationSummary: ExtendedConversationSummary = {
          id: conversation.id,
      ...
      console.error('Error starting study guide conversation:', error);
      set({ 
        error: error instanceof Error ? error.message : 'An unknown error occurred', 
        isLoading: false 
      });
  ...
  startPracticeQuestionsConversation: async (
    courseId, 
    topic, 
    questionCount = 5,
    ...
    try {
      const conversation = await conversationService.startPracticeQuestionsConversation(
        courseId, topic, questionCount, 
        difficulty as 'basic' | 'medium' | 'advanced', 
        questionTypes, moduleId, topicId
      );
      ...
      console.error('Error starting practice questions conversation:', error);
      set({ 
        error: error instanceof Error ? error.message : 'An unknown error occurred', 
        isLoading: false 
      });
  ...
  startKnowledgeGapConversation: async (
    courseId, 
    topic, 
    pastInteractionsCount = 10,
    moduleId, 
    topicId
  ) => {
    set({ isLoading: true, error: null });
    ...
    try {
      const conversation = await conversationService.startKnowledgeGapConversation(
        courseId, topic, pastInteractionsCount, moduleId, topicId
      );
      ...
      console.error('Error starting knowledge gap conversation:', error);
      set({ 
        error: error instanceof Error ? error.message : 'An unknown error occurred', 
        isLoading: false 
      });
